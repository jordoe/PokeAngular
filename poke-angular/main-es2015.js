...
let PokedexDetailExtendComponent = class PokedexDetailExtendComponent {
...
PokedexDetailExtendComponent.ctorParameters = () => [
...
let PokedexDetailComponent = class PokedexDetailComponent {
    constructor(route, location, pokedexService) {
        this.route = route;
...
PokedexDetailComponent.ctorParameters = () => [
...
let PokedexListComponent = class PokedexListComponent {
    constructor() {
    ...
    choosePokemon(pokemon) {
        ...
        if (this.pokemonList !== undefined) {
            this.currentListIndex = this.pokemonList.findIndex((x) => x.id === this.localPokemon.id);
        }
...
let PokedexMapComponent = class PokedexMapComponent {
    constructor(pokedexService, route) {
        this.pokedexService = pokedexService;
        this.route = route;
        this.randomLocation = [
    ...
    initMap() {
        ...
        const tiles = leaflet__WEBPACK_IMPORTED_MODULE_2__["tileLayer"]('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', {
...
PokedexMapComponent.ctorParameters = () => [
...
let PokedexPokeworldComponent = class PokedexPokeworldComponent {
    constructor(router, pokedexService) {
        this.router = router;
        this.pokedexService = pokedexService;
    ...
    initMap(pokemonArray) {
        ...
        const tiles = leaflet__WEBPACK_IMPORTED_MODULE_1__["tileLayer"]('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a>',
            subdomains: 'abcd',
            maxZoom: 19,
        });
    ...
    addIcon(pokemon) {
        ...
        const newIcon = leaflet__WEBPACK_IMPORTED_MODULE_1__["icon"]({
            iconUrl: pokemon.image,
            iconSize: [52, 52],
        });
        const marker = leaflet__WEBPACK_IMPORTED_MODULE_1__["marker"](randomLocation, {
            icon: newIcon,
...
PokedexPokeworldComponent.ctorParameters = () => [
...
PokedexComponent.ctorParameters = () => [
    { type: _Services_pokedex_service__WEBPACK_IMPORTED_MODULE_2__["PokedexService"] }
];
...
let PokedexService = class PokedexService {
    ...
    getPokemonList() {
        ...
        return data.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((res) => {
    ...
    getPokemonDetailsByName(id) {
        ...
        const obs = new rxjs__WEBPACK_IMPORTED_MODULE_3__["Observable"](observer => {
            this.http.get(url).subscribe((x) => {
                ...
                this.http.get(x.species.url).subscribe((y) => {
    ...
    getPokemonLearnMethod(id) {
        const movesLearnMethod = [];
        const url = 'http://pokeapi.co/api/v2/pokemon/' + id + '/';
        const obs = new rxjs__WEBPACK_IMPORTED_MODULE_3__["Observable"](observer => {
            this.http.get(url).subscribe((data) => {
                ...
                for (const moves of data.moves) {
                    observablesArr.push(this.http.get(moves.version_group_details[0].move_learn_method.url));
                    levelArr.push(moves.version_group_details[0].level_learned_at);
                }
                Object(rxjs__WEBPACK_IMPORTED_MODULE_3__["forkJoin"])(observablesArr).subscribe((result) => {
...
let FilterNamePipe = class FilterNamePipe {
    transform(pokemonList, name) {
        let result;
        if (name === undefined) {
            name = '';
        }
        if (pokemonList !== undefined) {
            result = pokemonList.filter(pokemon => pokemon.name.indexOf(name) !== -1);
        }
        return result;
    }
};
